...
const styles = `
:root{
  --bg:#f6f8fb; --card:#ffffff; --border:#e6e8ef; --muted:#6b7280; --text:#0f172a;
  --primary:#4f46e5; --primary-600:#5048f0; --warn:#f59e0b; --danger:#ef4444; --success:#16a34a;
}
...
.input-sm{ padding:.45rem .65rem; font-size:.9rem; }
...
/* Tabla */
.table-wrap{ background:var(--card); border:1px solid var(--border); border-radius:1rem; margin-top:1rem; overflow:auto;
  box-shadow:0 6px 20px rgba(15,23,42,.06); position:relative; z-index:1; }
...
.table .observaciones{ max-width:320px; white-space:normal; word-wrap:break-word; text-align:left; }

/* Badges */
.badge{ display:inline-block; padding:.25rem .6rem; border-radius:999px; font-weight:600; font-size:.85rem; background:#e5e7eb; }
...
.badge.method{ background:#eef2ff; color:#4338ca; } /* azul suave */
.badge.info{ background: rgba(59,130,246,.14); color:#2563eb; } /* info */
...
.form-grid input, .form-grid select, textarea{
  width:100%; padding:.6rem .8rem; border:1px solid var(--border); border-radius:.6rem; background:#fff; outline:none; color:var(--text);
}
...
.form-grid input:-webkit-autofill, .form-grid select:-webkit-autofill, textarea:-webkit-autofill{
